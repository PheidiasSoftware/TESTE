...
- Quando `Content-Length` é enviado, ele deve ser um inteiro decimal válido, não pode exceder `MAX_BODY_BYTES` e deve corresponder ao tamanho real do corpo recebido.
...
| --- | --- |
| `400` | JSON inválido, `Content-Length` inválido/incompatível, `task` ausente/vazia, `contextFiles` não-array, item de `contextFiles` não-textual ou caminho malformado. |
